...
function print_usage {
    ...
    usage_option " -a <score address> : SCORE address of the price feed score to remove"
    usage_footer
    exit 1
}
...
function process {
    ...
    command="tbears sendtx <(python ./scripts/score/dynamic_call/remove_feed.py ${network} ${score}) 
